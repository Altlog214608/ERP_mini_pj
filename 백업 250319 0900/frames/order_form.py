...
class order(tk.Frame):
  ...
  def create_order_form(self):  # 등록
    ...
    self.order_label = tk.Label(self.topleft_Frame, text="작성 일자", bg=Color.WHITE)
    ...
    self.order_date.grid(row=0, column=1, padx=10, pady=5)

    self.ordercode_label = tk.Label(self.topleft_Frame, text="발주 코드" , bg=Color.WHITE)
    ...
    self.order_entry.grid(row=1, column=1, padx=10, pady=5)

    self.inout_label = tk.Label(self.topleft_Frame, text="내/외부", bg=Color.WHITE)
    ...
    self.inout_entry.grid(row=2, column=1, padx=10, pady=5)

    self.order_author = tk.Label(self.topleft_Frame, text="작성자 명", bg=Color.WHITE)
    ...
    self.author_position_label = tk.Label(self.topleft_Frame, text="작성자 직책", bg=Color.WHITE)
    ...
    self.author_position_antry.grid(row=4, column=1, padx=10, pady=5)

    self.author_phone_label = tk.Label(self.topleft_Frame, text="작성자 연락처", bg=Color.WHITE)
    ...
    self.author_phone_entry.grid(row=5, column=1, padx=10, pady=5)

    self.author_email_label = tk.Label(self.topleft_Frame, text="작성자 이메일", bg=Color.WHITE)
    ...
    self.author_email_entry.grid(row=6, column=1, padx=10, pady=5)

    self.manager_label = tk.Label(self.topleft_Frame, text="관리자",bg=Color.WHITE)
    ...
    self.manager_entry.grid(row=7, column=1, padx=10, pady=5)

    self.manager_position_label = tk.Label(self.topleft_Frame, text="관리자 직책",bg=Color.WHITE)
    ...
    self.manager_position_entry.grid(row=8, column=1, padx=10, pady=5)

    self.manager_phone_label = tk.Label(self.topleft_Frame, text="관리자 연락처",bg=Color.WHITE)
    ...
    self.manager_phone_entry.grid(row=9, column=1, padx=10, pady=5)

    self.manager_email_label = tk.Label(self.topleft_Frame, text="관리자 이메일",bg=Color.WHITE)
    ...
    self.manager_email_entry.grid(row=10, column=1, padx=10, pady=5)

    self.product_label = tk.Label(self.topleft_Frame, text="완제품 명",bg=Color.WHITE)
    self.product_label.grid(row=0, column=2, padx=10, pady=5, sticky="w")
    self.product_entry = tk.Entry(self.topleft_Frame, width=25, state="disabled")
    self.product_entry.grid(row=0, column=3, padx=10, pady=5)

    self.unitprice_label = tk.Label(self.topleft_Frame, text="단가",bg=Color.WHITE)
    self.unitprice_label.grid(row=1, column=2, padx=10, pady=5, sticky="w")
    self.unitprice_entry = tk.Entry(self.topleft_Frame, state="disabled")
    self.unitprice_entry.grid(row=1, column=3, padx=10, pady=5)

    self.inventory_label = tk.Label(self.topleft_Frame, text="창고",bg=Color.WHITE)
    self.inventory_label.grid(row=2, column=2, padx=10, pady=5, sticky="w")
    self.inventory_entry = tk.Entry(self.topleft_Frame, state="disabled")
    self.inventory_entry.grid(row=2, column=3, padx=10, pady=5)

    self.quantity_label = tk.Label(self.topleft_Frame, text="거래 수량",bg=Color.WHITE)
    ...
    self.quantity_entry.grid(row=3, column=3, padx=10, pady=5)

    self.total_label = tk.Label(self.topleft_Frame, text="총 가격",bg=Color.WHITE)
    self.total_label.grid(row=4, column=2, padx=10, pady=5, sticky="w")
    self.total_entry = tk.Entry(self.topleft_Frame, state="disabled")
    self.total_entry.grid(row=4, column=3, padx=10, pady=5)

    self.VAT_label = tk.Label(self.topleft_Frame, text="부가세",bg=Color.WHITE)
    self.VAT_label.grid(row=5, column=2, padx=10, pady=5, sticky="w")
    self.VAT_antry = tk.Entry(self.topleft_Frame, state="disabled")
    self.VAT_antry.grid(row=5, column=3, padx=10, pady=5)

    self.correspondent_label = tk.Label(self.topleft_Frame, text="거래처 코드",bg=Color.WHITE)
    ...
    self.correspondent_combobox.grid(row=0, column=5, padx=10, pady=5)

    self.correspondent_name_label = tk.Label(self.topleft_Frame, text="거래처 명",bg=Color.WHITE)
    ...
    self.correspondent_name_entry.grid(row=1, column=5, padx=10, pady=5)

    self.type_label = tk.Label(self.topleft_Frame, text="거래처 종류",bg=Color.WHITE)
    self.type_label.grid(row=2, column=4, padx=10, pady=5, sticky="w")
    self.type_entry = tk.Entry(self.topleft_Frame, state="disabled")
    self.type_entry.grid(row=2, column=5, padx=10, pady=5)

    self.address_label = tk.Label(self.topleft_Frame, text="거래처 주소(국가)",bg=Color.WHITE)
    ...
    self.address_entry.grid(row=3, column=5, padx=10, pady=5)

    self.account_manager_label = tk.Label(self.topleft_Frame, text="거래처 담당자",bg=Color.WHITE)
    ...
    self.account_manager_entry.grid(row=4, column=5, padx=10, pady=5)

    self.account_phone_labe = tk.Label(self.topleft_Frame, text="거래처 담당자 연락처",bg=Color.WHITE)
    ...
    self.account_phone_entry.grid(row=5, column=5, padx=10, pady=5)

    self.account_email_label = tk.Label(self.topleft_Frame, text="거래처 담당자 이메일",bg=Color.WHITE)
    ...
    self.account_email_entry.grid(row=6, column=5, padx=10, pady=5)

    self.sledding_label = tk.Label(self.topleft_Frame, text="진행 상태",bg=Color.WHITE)
    self.sledding_label.grid(row=7, column=4, padx=10, pady=5, sticky="w")
    self.sledding_entry = tk.Entry(self.topleft_Frame, state="disabled")
    self.sledding_entry.grid(row=7, column=5, padx=10, pady=5)

    self.deadline_label = tk.Label(self.topleft_Frame, text="납기일",bg=Color.WHITE)
    self.deadline_label.grid(row=8, column=4, padx=10, pady=5, sticky="w")
    self.deadline_entry = DateEntry(self.topleft_Frame, width=22, background="#e3e3e3", foreground="white",
                                    date_pattern="yyyy-mm-dd", state="disabled")
    self.deadline_entry.grid(row=8, column=5, padx=10, pady=5)

    self.modification_label = tk.Label(self.topleft_Frame, text="수정 일자",bg=Color.WHITE)
    self.modification_label.grid(row=9, column=4, padx=10, pady=5, sticky="w")
    self.modification_entry = DateEntry(self.topleft_Frame, width=22, background="#e3e3e3", foreground="white",
                                        date_pattern="yyyy-mm-dd", state="disabled")
    self.modification_entry.grid(row=9, column=5, padx=10, pady=5)
    ...
    self.labela = tk.Label(self.topright_Frame, text="발주 코드",bg=Color.WHITE)
    self.labela.grid(row=0, column=0, padx=10, pady=5, sticky="w")
    self.allOrdering_entry = tk.Entry(self.topright_Frame)
    self.allOrdering_entry.grid(row=0, column=1, padx=10, pady=5, sticky="w")

    self.labelc = tk.Label(self.topright_Frame, text="완제품 명",bg=Color.WHITE)
    self.labelc.grid(row=1, column=0, padx=10, pady=5, sticky="w")
    self.allproduction_entry = tk.Entry(self.topright_Frame)
    self.allproduction_entry.grid(row=1, column=1, padx=10, pady=5, sticky="w")

    self.labeld = tk.Label(self.topright_Frame, text="거래처 코드",bg=Color.WHITE)
    self.labeld.grid(row=2, column=0, padx=10, pady=5, sticky="w")
    self.allCorrespondent_entry = tk.Entry(self.topright_Frame)
    self.allCorrespondent_entry.grid(row=2, column=1, padx=10, pady=5, sticky="w")

    self.labele = tk.Label(self.topright_Frame, text="관리자",bg=Color.WHITE)
    self.labele.grid(row=3, column=0, padx=10, pady=5, sticky="w")
    self.entryd = tk.Entry(self.topright_Frame)
    self.entryd.grid(row=3, column=1, padx=10, pady=5, sticky="w")

    self.labelz = tk.Label(self.topright_Frame, text="납기일",bg=Color.WHITE)
    self.labelz.grid(row=4, column=0, padx=10, pady=5, sticky="w")
    self.date_entry4 = DateEntry(self.topright_Frame, width=10, background="#e3e3e3", foreground="white", date_pattern="yyyy-mm-dd")
    self.date_entry4.grid(row=4, column=1, padx=10, pady=6, sticky="nsew")  # sticky="nsew"로 모든 방향에 맞게 채우기

    self.date_entry5 = DateEntry(self.topright_Frame, width=10, background="#e3e3e3", foreground="white", date_pattern="yyyy-mm-dd")
    self.date_entry5.grid(row=4, column=2, padx=10, pady=6, sticky="nsew")  # sticky="nsew"로 모든 방향에 맞게 채우기

    self.labelf = tk.Label(self.topright_Frame, text="작성일자",bg=Color.WHITE)
    self.labelf.grid(row=5, column=0, padx=10, pady=5, sticky="w")

    self.creation_date_start = DateEntry(self.topright_Frame, width=22, background="#e3e3e3", foreground="white",
                                         date_pattern="yyyy-mm-dd")
    self.creation_date_start.grid(row=5, column=1, padx=10, pady=5, sticky="w")

    self.creation_date_end = DateEntry(self.topright_Frame, width=22, background="#e3e3e3", foreground="white",
                                       date_pattern="yyyy-mm-dd")
    self.creation_date_end.grid(row=5, column=2, padx=10, pady=5, sticky="w")

    self.labelg = tk.Label(self.topright_Frame, text="수정 일자",bg=Color.WHITE)
    self.labelg.grid(row=6, column=0, padx=10, pady=5, sticky="w")

    self.modified_date_start = DateEntry(self.topright_Frame, width=30, background="#e3e3e3", foreground="white",
                                         date_pattern="yyyy-mm-dd")
    self.modified_date_start.grid(row=6, column=1, padx=10, pady=5)

    self.modified_date_end = DateEntry(self.topright_Frame, width=30, background="#e3e3e3", foreground="white",
                                       date_pattern="yyyy-mm-dd")
    self.modified_date_end.grid(row=6, column=2, padx=10, pady=5)

    self.btn_search = tk.Button(self.topright_Frame, text="조회", command=self.select_point ,bg=Color.WHITE)
    self.btn_search.grid(row=0, column=3, padx=10, pady=5, sticky="e")

    # 눌러야 활성화 되면서 작성 가능하게
    self.btn_create = tk.Button(self.topright_Frame, text="생성", command=self.create_new_entry ,bg=Color.WHITE)
    self.btn_create.grid(row=1, column=3, padx=10, pady=5, sticky="e")

    # 눌러야 활성화 되면서 작성 가능하게
    self.btn_update = tk.Button(self.topright_Frame, text="수정", command=self.edit_existing_entry ,bg=Color.WHITE)
    self.btn_update.grid(row=2, column=3, padx=10, pady=5, sticky="e")

    # 생성과 등록 모두가 사용
    self.btn_save = tk.Button(self.topright_Frame, text="저장", command=self.create_point ,bg=Color.WHITE)
    self.btn_save.grid(row=3, column=3, padx=10, pady=5, sticky="e")

    self.btn_delete = tk.Button(self.topright_Frame, text='삭제', command=self.enable_order_entry, bg=Color.WHITE)
    self.btn_delete.grid(row=4, column=3, padx=10, pady=5, sticky="e")
  ...
  #     'Customer_code', 'account_name', 'account_type', 'account_manager', 'delivery_date', 'modified_date'
  #   ]
  #
  #   # 기본 쿼리
  ...
  #     'order_vat', 'Customer_code', 'account_name', 'account_type', 'account_address',
