...
class TaxInvoiceFrame(tk.Frame):
    def __init__(self, root):
        ...
        self.tiSearchBtn = ttk.Button(self.tiControlFrame, text='조회하기', width=10, command=self.searchBtn)
        self.tiSearchBtn.place(x=1200, y=5)

        self.tiSaveBtn = ttk.Button(self.tiControlFrame, text='저장하기', width=10, command=saveTi)
        self.tiSaveBtn.place(x=1200, y=32)

        self.tiPublishBtn = ttk.Button(self.tiControlFrame, text='발행하기', width=10, command=publishTi)
    ...
    def drawScTable(self, key, cont):
        if key == 'subject':
            ...
            self.cancelBtn = ttk.Button(self.scFrame, text='취소', width=8, command=cancel)
            self.cancelBtn.place(x=430, y=400)
            self.scFrame.place(x=300, y=70)
            self.scTable.focus_set()
            self.scTable.bind('<Escape>', lambda e: cancel())
