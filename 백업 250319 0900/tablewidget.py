...
class TableWidget(tk.Frame):
    def __init__(self, master, padding=0, data=None, cols=0, new_row=True, has_checkbox=True, col_name=None,
        ...
        self.canvas = tk.Canvas(self, width=self.width - 2 * padding, height=self.height - self.scrollbar_width - 2 * padding, highlightthickness=0)  # canvas
    ...
    def from_data(self, data, cols = 0, new_row = True, has_checkbox = True,
        ...
        self.canvas.config(scrollregion=(0, 0, self.origin_cell_width + self.checkbox_cell_width + sum(self.col_widths) + 2 * self.padding, self.origin_cell_height + self.rows * self.cell_height))
    ...
    def refresh(self, data):
        self.from_data(data=data, new_row=self.new_row, has_checkbox=self.has_checkbox, col_name=self.col_name, col_width=self.col_widths, col_align=self.col_align, editable=self.editable)
    ...
    def draw_table(self):
        ...
        # 좌상단 셀
        self.canvas.create_rectangle(0, 0, self.origin_cell_width, self.origin_cell_height * self.col_name.rows, outline=Color.BLACK, fill=Color.GRAY, tags="header")

        # 체크박스 열
        if self.has_checkbox:
            self.canvas.create_rectangle(self.origin_cell_width, 0, self.origin_cell_width + self.checkbox_cell_width, self.origin_cell_height * self.col_name.rows, outline=Color.BLACK, fill=Color.GRAY, tags="header")
        ...
        for title in self.col_name.names:
            ...
            self.canvas.create_rectangle(x1, y1, x2, y2, outline=Color.BLACK, fill=Color.GRAY, tags="header")
        ...
        for row in range(self.row_count):
            ...
            self.canvas.create_rectangle(0, y1, self.origin_cell_width, y2, outline=Color.BLACK, fill=Color.GRAY,
                                         tags="header")
            ...
            for col in range(self.cols):
                ...
                self.canvas.create_rectangle(x1, y1, x2, y2, outline=Color.BLACK,
                                             fill="#f0f0f0" if current_row != self.selected_row or not self.editable[
    ...
    def edit_cell(self):
        ...
        if self.has_checkbox and self.selected_col < 0:
            ...
            self.entry.place(x=self.padding + self.origin_cell_width + self.checkbox_cell_width + sum(self.col_widths[:self.selected_col]),
