...
class Main(tk.Tk):
    def __init__(self):
        ...
        self.la_logo = tk.Label(self.fr_logo, image=self.im_logo, width=300, height=130, bg=Color.WHITE)
        self.la_logo.bind("<Button-1>", lambda e: self.screen_main())
        self.la_logo.place(x=0, y=0)
        ...
        self.etc_buttons.append(
            tk.Button(self.fr_etc_inner, text="e006 테스트", bg=Color.FOCUS, relief="flat", padx=30, pady=5,
        ...
        # HorizontalLine(self, 1280, Color.BLACK).place(x=310, y=480)
        # VerticalLine(self, 330, Color.BLACK).place(x=1250, y=140)
    ...
    def draw_nt(self):
        if not self.logged_in:
            ...
            return
        if self.nt_flag:
            self.nt_frame.place(x=1250, y=100)
