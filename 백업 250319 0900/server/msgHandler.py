...
##창고조회############################################
    ...
    def f40501(**kwargs):
        ...
        # accountsubject 테이블에서 account_name과 account_type 조회
        account_data = dbm.query(
            "SELECT account_name, account_type FROM accountsubject WHERE account_type IN ('비용','수익')"
        )
        ...
        for i in a:
            ...
            jr_dr = i[1]  # SUM(jr_dr)
            jr_cr = i[2]  # SUM(jr_cr)
            account_type = account_dict.get(account_name, None)  # 과목별 (name) 수익 , 비용
    ...
    def f81001(**kwargs):
        ...
        print(req)
        # sql
        query = dbm.query("SELECT name, job_grade from employee where employee_code = %(id)s", {
            "id": id_
        })
