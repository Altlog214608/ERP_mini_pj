...
class Main(tk.Tk):
    def __init__(self):
        ...
        self.category = {
            ...
            "회계": {
                ...
                "관리회계": {
                    "생산비용 분석": [Production_cost_analysis_1.Production_cost_analysis_1, Production_cost_analysis_2.Production_cost_analysis_2]
                },
            },
        }
        ...
        self.lb_logo = tk.Label(self.fr_logo, image=self.im_logo, width=300, height=130, borderwidth=0, highlightthickness=0, bg=Color.WHITE)
        ...
        self.bt_login = tk.Button(self.fr_notification, image=self.im_login, relief="flat", bg=Color.WHITE, command=self.login)

        self.im_logout = tk.PhotoImage(file="sign-out-alt.png")
        self.bt_logout = tk.Button(self.fr_notification, image=self.im_logout, relief="flat", bg=Color.WHITE, command=self.logout)
        ...
        self.etc_buttons.append(tk.Button(self.fr_etc_inner, text="e001>e002 메시지", bg=Color.FOCUS, relief="flat", padx=30, pady=5, command=self.start_work))
        ...
        self.etc_buttons.append(tk.Button(self.fr_etc_inner, text="e001>e006 결재", bg=Color.FOCUS, relief="flat", padx=30, pady=5, command=self.appr))
        ...
        for first in self.category:
            first_tree = Category(tk.Button(self.fr_menu, text=first, anchor="w", bg=Color.FOCUS, relief="flat", command=lambda x=first: self.select_category(0, x)))
            ...
            for second in self.category[first]:
                second_tree = Category(tk.Button(self.fr_menu, text=f"　　{second}", anchor="w", bg=Color.GRAY, relief="flat", command=lambda x=second: self.select_category(1, x)))
                first_tree.add(second, second_tree)

                for third in self.category[first][second]:
                    third_tree = Category(tk.Button(self.fr_menu, text=f"　　　　{third}", anchor="w", bg=Color.WHITE, relief="flat", command=lambda x=third: self.select_category(2, x)))
    ...
    def draw_pages(self):
        for b in self.footer_buttons:
            b.destroy()
        self.footer_buttons.clear()
        self.footer_buttons.append(tk.Button(self.fr_footer, text="<", bg=Color.WHITE, padx=10, pady=10, relief="flat", command=self.prev_page))
        for i in range(self.pages):
            self.footer_buttons.append(tk.Button(self.fr_footer, text=str(i + 1), bg=Color.WHITE, padx=10, pady=10, relief="flat", command=lambda x=i: self.select_page(x)))
            if self.current_page == i:
                self.footer_buttons[-1].config(bg=Color.GRAY)
            else:
                self.footer_buttons[-1].config(bg=Color.WHITE)
        self.footer_buttons.append(tk.Button(self.fr_footer, text=">", bg=Color.WHITE, padx=10, pady=10, relief="flat", command=self.next_page))
    ...
    def append_tab(self, first, second, third):
        ...
        tab.button = tk.Button(self.fr_tabs, text=tab.third, height=30, padx=10, relief="flat", command=lambda: self.select_tab(tab))
