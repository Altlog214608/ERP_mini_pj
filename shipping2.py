...
class Shipping(tk.Frame):
    ...
    def check_data(self): #데이터 조회 버튼
        if self.tentry1.get():
            ...
            self.sub_datalist = self.DataBase.query(self.DataBase,f"SELECT * FROM purchase_orders where purchase_order_code = '{target}'")
            self.sub_data = [[f"{c}" for c in self.sub_datalist[r]] for r in range(len(self.sub_datalist))]
            # print(self.sub_data)
            self.sub_table_columns = self.get_columns("purchase_orders")
    ...
    def make_table(self):
        # self.DataBase.query(self.DataBase,"use sakila")
        ...
        #     )""")
        self.DataBase.query(self.DataBase,
            """
            create table IF NOT EXISTS testShipping(
            shipping_code INT NOT NULL AUTO_INCREMENT,
            production_order char(10) NOT NULL,
        ...
        print(self.DataBase.query(self.DataBase,"SHOW TABLES"))
        # print(self.dbm.query(self.DataBase,"SHOW TABLES"))
    ...
    def get_all_data(self, tablename):
        ...
        result = self.DataBase.query(self.DataBase,f"SELECT * FROM {tablename}")
        return list(result)
    ...
    def get_columns(self, tablename):
        ...
        result = self.DataBase.query(self.DataBase,f"SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS where TABLE_SCHEMA = 'sakila' AND TABLE_NAME  = '{tablename}' ORDER BY ORDINAL_POSITION;")
        print(result)
        for i in result:
    ...
    def add_column(self, tableName, name, type, size, null=None):
        ...
        # """
        #                               )
        check_column = self.DataBase.query(self.DataBase,f"""
        SELECT COUNT(*)
        FROM INFORMATION_SCHEMA.COLUMNS
        WHERE TABLE_SCHEMA = 'test' 
        AND TABLE_NAME = '{tableName}' 
        ...
        if check_column[0][0] == 0:
            if "NOT" in null:
                # self.DataBase.query(f"ALTER TABLE {tableName} ADD COLUMN {name} {type}({size}) NOT NULL")
                # self.dbm.query(f"ALTER TABLE {tableName} ADD COLUMN {name} {type}({size}) NOT NULL")
                print(f"낫 널 {name}컬럼 추가")
            else:
                # self.DataBase.query(f"ALTER TABLE {tableName} ADD COLUMN {name} {type}({size})")
